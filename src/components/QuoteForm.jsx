    ...
    <div className="card p-4 space-y-3">
      ...
      <div ref={wrapperRef} className="relative">
        ...
        {showDropdown && filtered.length > 0 && (
          <div className="absolute z-30 top-full left-0 right-0 mt-1 card border-slate-700 shadow-xl overflow-hidden">
            {filtered.map(c => (
              <button
                key={c.id}
              ...
              >
                ...
                <p className="text-xs text-slate-500">
                  {[c.city, c.piva ? `P.IVA ${c.piva}` : null].filter(Boolean).join(' · ')}
                </p>
              </button>
            ))}
          </div>
